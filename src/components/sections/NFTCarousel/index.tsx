...
export const NFTCarousel = () => {
  ...
  const [videoPlayingSlot, setVideoPlayingSlot] = useState<number | null>(null); // Слот, где играет видео
  ...
  const spin = () => {
    ...
    setSpinning(true);
    setSelectedSlot(null); // Сбрасываем текущий выбранный слот
      ...
      setTimeout(() => {
        ...
        if (index === 2) {
          ...
          setTimeout(() => {
            ...
            setVideoPlayingSlot(randomSlot); // Устанавливаем слот, где будет играть видео
            ...
            if (videoRefs[randomSlot]?.current) {
              ...
              videoEl.onended = () => {
                setVideoPlayingSlot(null); // Убираем слот, когда видео заканчивается
              };
            }
  ...
  useEffect(() => {
    spin(); // Запуск первого спина при загрузке компонента
        ...
        return (
          <div
            ...
            className={cn(
              ...
              "border-[4px] sm:border-[6px] border-white", // Адаптивный внутренний бордер
              "shadow-slot",
