// src/components/dialogs/UploadDialog/index.tsx
...
export default function UploadDialog({ onClose, promptData }: UploadDialogProps) {
    ...
    const [uploadError, setUploadError] = useState<string | null>(null); // Добавляем состояние для ошибки
...
useEffect(() => {
  ...
  const extractTraitsFromPrompt = (promptText: string) => {
    ...
    // Не нужно else, т.к. patternColor уже инициализирован как 'UNKNOWN'
    }
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type !== 'image/jpeg') {
        alert('Please upload JPEG files only (file type: ' + file.type + ')');
        return;
    }
    ...
    try {
      ...
      // Здесь будет загрузка на сервер, когда мы добавим эту функциональность
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        alert('Image uploaded successfully! It will be reviewed by moderators.');
        onClose();
    } catch (error: any) {
