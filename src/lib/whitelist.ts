...
export const checkWhitelistEntries = (params: {
 ...
 try {
   ...
   return {
     address: params.address ? entries.some(e => e.w.toLowerCase() === params.address?.toLowerCase()) : false,
     discord: params.discord ? entries.some(e => e.d === params.discord) : false,
     twitter: params.twitter ? entries.some(e => e.t.toLowerCase() === params.twitter?.toLowerCase()) : false
   }
 } catch (error) {
...
export const addToWhitelist = async (entry: {
 address: string,
 discord: string,
 twitter: string
}) => {
 try {
   ...
   const formattedEntry: WhitelistEntry = {
     w: entry.address,
     d: entry.discord,
     t: entry.twitter
   };
